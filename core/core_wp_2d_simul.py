...
def _is_notebook_environment():
    """Return True when the code is running inside a Jupyter-style frontend."""
    try:
        shell_name = get_ipython().__class__.__name__                                    # Read the active IPython shell name
    except NameError:
        return False                                                                     # Standard Python scripts do not define get_ipython

    return shell_name == "ZMQInteractiveShell"                                           # Jupyter notebooks and VS Code cells use this shell


def _is_matplotlib_gui_backend():
    """Return True when Matplotlib is using a backend that can open GUI windows."""
    backend_name = matplotlib.get_backend().lower()                                      # Read current Matplotlib backend
    non_gui_backends = ("agg", "pdf", "svg", "ps", "cairo", "template")                  # File-only backends cannot display windows
    return not any(backend in backend_name for backend in non_gui_backends)              # Keep GUI display only for interactive backends
...
class FFt_src:
    ...
    def fft_src(self):
        ...
        if not _is_notebook_environment() and _is_matplotlib_gui_backend():                    # Show source figure when running as a .py script
            fig.show()                                                                         # Open the Matplotlib window without blocking the simulation
            fig.canvas.draw_idle()                                                             # Request a first draw of the source figure
            fig.canvas.flush_events()                                                          # Push pending GUI events to the window
            plt.pause(0.001)                                                                   # Give the GUI backend time to paint the figure
...
class animation2D_FDM:
    ...
    def __init__(
        ...
        sample_metadata=None,          # Optional dictionary with sample metadata
        #--------------------------------------------------------
        source_events=None,            # Optional delayed multi-source rupture events
        #--------------------------------------------------------
        
        # -------------------------
        # PyVista snapshots
        # -------------------------
        store_snapshots=False,         # If True, store wavefield snapshots for PyVista visualization
        snapshot_interval=10,          # Save one wavefield snapshot every N time steps
        snapshot_dtype=np.float32,     # Data type used to store snapshots efficiently
        snapshot_h5_path=None,         # Optional HDF5 file path for persistent PyVista snapshots
        save_snapshots_to_h5=False     # If True, write the PyVista snapshots dictionary to snapshot_h5_path
        #------------
    ):
        self.sample = sample
        self.nx = nx
        ...
        self.src = src
        #--------------------------------------------------------
        self.source_events = self._prepare_source_events(source_events)                         # Build one validated source-event list without breaking the old single-source mode
        #--------------------------------------------------------
        self.idisp = idisp
        self.nop = nop
        self.fsc = fsc
        ...
        # PyVista snapshots
        # -------------------------
        self.store_snapshots = bool(store_snapshots)                                             # Store wavefield snapshots only when explicitly requested
        self.snapshot_interval = int(snapshot_interval)                                          # Store snapshot interval in time steps
        self.snapshot_dtype = snapshot_dtype                                                     # Store snapshot data type
        self.snapshot_h5_path = None if snapshot_h5_path is None else Path(snapshot_h5_path)      # Optional independent HDF5 file for PyVista playback
        self.save_snapshots_to_h5 = bool(save_snapshots_to_h5)                                    # Persist snapshots only when explicitly requested

        if self.save_snapshots_to_h5 and not self.store_snapshots:                                # HDF5 snapshot output requires stored wavefield frames
            raise ValueError("save_snapshots_to_h5=True requires store_snapshots=True.")

        if self.save_snapshots_to_h5 and self.snapshot_h5_path is None:                           # Avoid silently writing to an unknown location
            raise ValueError("snapshot_h5_path must be provided when save_snapshots_to_h5=True.")

        if self.snapshot_interval <= 0:                                                          # Validate snapshot interval
            raise ValueError("snapshot_interval must be a positive integer.")                    # Stop if snapshot interval is invalid
        #-----------

    #--------------------------------------------------------
    def _prepare_source_events(self, source_events):                                             # Normalize optional rupture events into one internal source-event list
        base_src = np.asarray(self.src, dtype=float)                                             # Convert the original source signal to a numeric array
        if base_src.ndim != 1:                                                                   # Validate that the source signal is one-dimensional
            raise ValueError("src must be a one-dimensional source time function.")              # Stop if the base source cannot be indexed by time step
        if base_src.size == 0:                                                                   # Validate that the source signal has at least one sample
            raise ValueError("src cannot be empty.")                                             # Stop if the base source is empty

        if source_events is None:                                                               # Preserve the original single-source behavior when no rupture list is provided
            source_events = [{"x": self.isx, "z": self.isz, "src": base_src, "delay_s": 0.0, "amplitude": 1.0, "polarity": 1.0}]  # Build the legacy source as one event
            default_amplitude = 1.0                                                             # Keep the old source amplitude unchanged for legacy simulations
        else:                                                                                   # Use the user-provided rupture events when they are available
            source_events = list(source_events)                                                 # Materialize the rupture event iterable so its length is known
            if len(source_events) == 0:                                                         # Validate that the rupture event list is not empty
                raise ValueError("source_events cannot be empty.")                              # Stop if the user provided an empty rupture definition
            default_amplitude = 1.0 / float(len(source_events))                                 # Split the base source amplitude across events when amplitude is not provided

        prepared_events = []                                                                    # Store validated source events for the simulation loop
        for event_index, event in enumerate(source_events):                                      # Loop over each rupture event definition
            if not isinstance(event, dict):                                                     # Validate the event container type
                raise TypeError("Each source event must be a dictionary.")                      # Stop if the event cannot be read by key

            event_src = np.asarray(event.get("src", base_src), dtype=float)                     # Use an event-specific source or the original base source
            if event_src.ndim != 1:                                                             # Validate that the event source is one-dimensional
                raise ValueError("Each source event 'src' must be one-dimensional.")            # Stop if the event source cannot be indexed by time step
            if event_src.size == 0:                                                             # Validate that the event source has at least one sample
                raise ValueError("Each source event 'src' cannot be empty.")                    # Stop if the event source is empty

            event_x = int(event.get("x", event.get("source_x", self.isx)))                     # Read the event x index while supporting a source_x alias
            event_z = int(event.get("z", event.get("source_z", self.isz)))                     # Read the event z index while supporting a source_z alias
            if event_x < 0 or event_x >= int(self.nx):                                          # Validate the event x index against the model width
                raise ValueError(f"source_events[{event_index}] x is out of range [0, {int(self.nx) - 1}].")  # Stop if the x index is outside the grid
            if event_z < 0 or event_z >= int(self.nz):                                          # Validate the event z index against the model depth
                raise ValueError(f"source_events[{event_index}] z is out of range [0, {int(self.nz) - 1}].")  # Stop if the z index is outside the grid

            event_delay_s = float(event.get("delay_s", 0.0))                                    # Read the rupture delay in seconds
            if event_delay_s < 0.0:                                                             # Validate that rupture delay is not negative
                raise ValueError("source event delay_s must be non-negative.")                  # Stop if a rupture event starts before the simulation
            event_delay_samples = int(round(event_delay_s / float(self.dt)))                    # Convert delay in seconds to an integer time-step shift

            event_amplitude = float(event.get("amplitude", default_amplitude))                  # Read event amplitude or use the safe split-amplitude default
            event_polarity = float(event.get("polarity", 1.0))                                  # Read event polarity multiplier with positive default
            event_effective_src = event_amplitude * event_polarity * event_src                  # Build the final source wavelet injected by this event

            prepared_events.append(                                                             # Add the normalized event dictionary used by animate()
                {
                    "x": event_x,                                                               # Store event x index
                    "z": event_z,                                                               # Store event z index
                    "src": event_effective_src,                                                 # Store scaled and polarity-adjusted source wavelet
                    "delay_s": event_delay_s,                                                   # Store event delay in seconds
                    "delay_samples": event_delay_samples,                                       # Store event delay in time samples
                    "amplitude": event_amplitude,                                               # Store event amplitude multiplier
                    "polarity": event_polarity,                                                 # Store event polarity multiplier
                }
            )

        return prepared_events                                                                  # Return the validated rupture-event list
    #--------------------------------------------------------
    ...
    def animate(self):
        ...
        src = self.src
        #--------------------------------------------------------
        source_events = self.source_events                                                       # Read the normalized source-event list prepared during initialization
        source_primary_x = int(source_events[0]["x"])                                            # Keep the first source event as the legacy source_x reference
        source_primary_z = int(source_events[0]["z"])                                            # Keep the first source event as the legacy source_z reference
        source_x_all = np.asarray([event["x"] for event in source_events], dtype=np.int32)       # Store all rupture source x indices for metadata
        source_z_all = np.asarray([event["z"] for event in source_events], dtype=np.int32)       # Store all rupture source z indices for metadata
        source_delay_s = np.asarray([event["delay_s"] for event in source_events], dtype=np.float64)  # Store all rupture source delays in seconds
        source_delay_samples = np.asarray([event["delay_samples"] for event in source_events], dtype=np.int32)  # Store all rupture source delays in time samples
        source_amplitude = np.asarray([event["amplitude"] for event in source_events], dtype=np.float64)  # Store all rupture source amplitude multipliers
        source_polarity = np.asarray([event["polarity"] for event in source_events], dtype=np.float64)  # Store all rupture source polarity multipliers
        n_source_events = int(len(source_events))                                                # Store the number of rupture source events
        #--------------------------------------------------------
        idisp = self.idisp
        nop = self.nop
        show = self.show
        ...
        # PyVista snapshots
        # -------------------------
        store_snapshots = self.store_snapshots                                                   # Read snapshot storage flag
        snapshot_interval = self.snapshot_interval                                               # Read snapshot interval
        snapshot_dtype = self.snapshot_dtype                                                     # Read snapshot data type
        wavefield_snapshots = []                                                                 # List to store selected wavefield snapshots
        snapshot_time_indices = []                                                               # List to store corresponding time-step indices
        #------------
        ...
        cmax = float(np.max(c))
        CFL = cmax * dt / dx
        print("Courant-Friedrichs-Lewy criterion (CFL) :")
        print(CFL)
        ...
        # -------------------------
        #--------------------------------------------------------
        source_peak_values = [float(np.max(np.abs(event["src"]))) for event in source_events]    # Measure the peak amplitude of each effective source event
        v = max(source_peak_values) if len(source_peak_values) else 1.0                          # Use the strongest effective source for the wavefield color scale
        #--------------------------------------------------------
        if v == 0:
            v = 1.0
        ...
        #--------------------------------------------------------
        ax1.scatter(source_x_all, source_z_all, marker='*', s=150, color=(0, 0, 0))              # Plot every rupture source event instead of only one source marker
        for source_index, (source_x_i, source_z_i) in enumerate(zip(source_x_all, source_z_all)): # Label each rupture source event in the wavefield panel
            source_label = "Source" if n_source_events == 1 else f"R{source_index + 1}"           # Use the old label for one source and rupture labels for multiple sources
            ax1.text(                                                                            # Draw one source-event label next to its marker
                float(source_x_i) * 1.05, float(source_z_i), source_label,                       # Place the label near the current source marker
                ha='left', va='center', fontweight='bold', color=(0, 0, 0)                       # Keep the previous source-label style
            )
        #--------------------------------------------------------
        ...
        # -------------------------
        handle = None
        notebook_display = show and _is_notebook_environment()                                # Use notebook display updates inside Jupyter frontends
        script_display = show and not notebook_display and _is_matplotlib_gui_backend()        # Use GUI window updates when running as a .py script
        was_interactive = plt.isinteractive()                                                  # Preserve the user's original Matplotlib interactive state

        if show:
            if notebook_display:
                handle = display(fig, display_id=True)                                        # Create an updatable display handle for notebooks

            elif script_display:
                plt.ion()                                                                      # Enable live GUI updates for standard Python scripts
                fig.show()                                                                     # Open the animation window without blocking the simulation
                fig.canvas.draw_idle()                                                         # Request the first draw before the loop starts
                fig.canvas.flush_events()                                                      # Push pending GUI events to the window
                plt.pause(0.001)                                                               # Give the GUI backend time to paint the figure
        ...
        def update_plot(it):
            ...
            if notebook_display and handle is not None:
                handle.update(fig)

            elif script_display:
                fig.canvas.draw_idle()                                                         # Request a redraw after updating wavefield and seismograms
                fig.canvas.flush_events()                                                      # Push pending GUI events to keep the window responsive
                plt.pause(0.001)                                                               # Let Matplotlib refresh the live script window
        ...
        def run_simulation():
            ...
            for it in range(nt):
                ...
                #--------------------------------------------------------
                # source events
                for source_event in source_events:                                             # Inject every active rupture source event at the current global time step
                    source_local_it = it - int(source_event["delay_samples"])                   # Convert global time step to the local time index of this event
                    if 0 <= source_local_it < len(source_event["src"]):                         # Inject only while the delayed event source wavelet is active
                        pnew[source_event["z"], source_event["x"]] += source_event["src"][source_local_it]  # Add the delayed source amplitude at its grid position
                #--------------------------------------------------------
                ...
                # PyVista snapshots
                if store_snapshots and (it % snapshot_interval) == 0:                            # Store wavefield only at selected time steps
                    wavefield_snapshots.append(pnew.astype(snapshot_dtype, copy=True))           # Save current pressure field snapshot
                    snapshot_time_indices.append(it)                                             # Save corresponding time-step index
        ...
        if not show:
            ...
            fig.canvas.draw_idle()                                                             # Draw the final animation state before blocking
            fig.canvas.flush_events()                                                          # Flush pending events before the final show call
            if not was_interactive:
                plt.ioff()                                                                     # Restore non-interactive mode for scripts that started that way
            plt.show(block=True)                                                               # Keep script figures open until the user closes them
        ...
        results = {
            ...
            "nz": int(nz),
            "nt": int(nt),
            "source_x": int(source_primary_x),                                                  # Preserve the legacy primary source x field for existing training code
            "source_z": int(source_primary_z),                                                  # Preserve the legacy primary source z field for existing training code
            #--------------------------------------------------------
            "source_x_all": source_x_all.copy(),                                                # Store all rupture source x indices without changing the old source_x field
            "source_z_all": source_z_all.copy(),                                                # Store all rupture source z indices without changing the old source_z field
            "source_delay_s": source_delay_s.copy(),                                            # Store all rupture source delays in seconds
            "source_delay_samples": source_delay_samples.copy(),                                # Store all rupture source delays in time samples
            "source_amplitude": source_amplitude.copy(),                                        # Store all rupture source amplitude multipliers
            "source_polarity": source_polarity.copy(),                                          # Store all rupture source polarity multipliers
            "n_source_events": int(n_source_events),                                            # Store the number of rupture source events
            #--------------------------------------------------------
            "receiver_x": irx.copy(),
            "receiver_z": irz.copy(),
        ...
        if store_snapshots:
            snapshots = {
                "wavefield_snapshots": np.asarray(wavefield_snapshots, dtype=snapshot_dtype),
                "snapshot_time_indices": np.asarray(snapshot_time_indices, dtype=np.int32),
                "snapshot_times": np.asarray(snapshot_time_indices, dtype=float) * float(dt),
                "velocity_model": c.copy(),
                "dx": float(dx),
                "dz": float(dz),
                "dt": float(dt),
                ...
                "nz": int(nz),
                "nt": int(nt),
                "source_x": int(source_primary_x),                                              # Preserve the legacy primary source x field for existing PyVista code
                "source_z": int(source_primary_z),                                              # Preserve the legacy primary source z field for existing PyVista code
                #--------------------------------------------------------
                "source_x_all": source_x_all.copy(),                                            # Store all rupture source x indices for PyVista snapshot metadata
                "source_z_all": source_z_all.copy(),                                            # Store all rupture source z indices for PyVista snapshot metadata
                "source_delay_s": source_delay_s.copy(),                                        # Store all rupture source delays in seconds for PyVista snapshot metadata
                "source_delay_samples": source_delay_samples.copy(),                            # Store all rupture source delays in time samples for PyVista snapshot metadata
                "source_amplitude": source_amplitude.copy(),                                    # Store all rupture source amplitude multipliers for PyVista snapshot metadata
                "source_polarity": source_polarity.copy(),                                      # Store all rupture source polarity multipliers for PyVista snapshot metadata
                "n_source_events": int(n_source_events),                                        # Store the number of rupture source events for PyVista snapshot metadata
                #--------------------------------------------------------
                "receiver_x": irx.copy(),
                "receiver_z": irz.copy(),
                "model_type": str(self.model_type),
            ...
            # -------------------------
            # Optional persistent PyVista snapshot HDF5
            # -------------------------
            if self.save_snapshots_to_h5:
                snapshot_writer = HDF5PyVistaSnapshotWriter(                                  # Keep this file separate from the training HDF5
                    h5_path=self.snapshot_h5_path,
                    snapshot_dtype=snapshot_dtype,
                )
                snapshot_writer.append_sample(                                                # Store the complete viewer-ready snapshot dictionary
                    snapshots=snapshots,
                    sample=sample,
        ...
        # return results
        if store_snapshots:                                                                       # Return PyVista snapshots only when explicitly requested
            return results, snapshots                                                             # Keep results unchanged and add a separate visualization dictionary

        return results                                                                            # Preserve the original output when snapshots are not requested
...
class HDF5SurfaceSeismogramWriter:
    ...
    def append_sample(self, results):
        ...
        with h5py.File(self.h5_path, "a") as h5f:
            ...
            self._append_scalar_dataset(g_metadata, "CFL", results["CFL"], dtype=np.float64)
            ...
            #--------------------------------------------------------
            if "n_source_events" in results:                                                    # Store the source-event count only when the simulation result provides it
                self._append_scalar_dataset(g_metadata, "n_source_events", results["n_source_events"], dtype=np.int32)  # Keep one count per sample for compatibility with variable files
            if "source_x_all" in results:                                                        # Store all rupture source x indices when available
                self._write_constant_array(g_metadata, "source_x_all", results["source_x_all"], dtype=np.int32)  # Keep source geometry fixed across the HDF5 dataset
            if "source_z_all" in results:                                                        # Store all rupture source z indices when available
                self._write_constant_array(g_metadata, "source_z_all", results["source_z_all"], dtype=np.int32)  # Keep source geometry fixed across the HDF5 dataset
            if "source_delay_s" in results:                                                      # Store all rupture source delays in seconds when available
                self._write_constant_array(g_metadata, "source_delay_s", results["source_delay_s"], dtype=np.float64)  # Keep rupture timing fixed across the HDF5 dataset
            if "source_delay_samples" in results:                                                # Store all rupture source delays in samples when available
                self._write_constant_array(g_metadata, "source_delay_samples", results["source_delay_samples"], dtype=np.int32)  # Keep integer rupture timing fixed across the HDF5 dataset
            if "source_amplitude" in results:                                                    # Store all rupture source amplitudes when available
                self._write_constant_array(g_metadata, "source_amplitude", results["source_amplitude"], dtype=np.float64)  # Keep rupture amplitudes fixed across the HDF5 dataset
            if "source_polarity" in results:                                                     # Store all rupture source polarities when available
                self._write_constant_array(g_metadata, "source_polarity", results["source_polarity"], dtype=np.float64)  # Keep rupture polarities fixed across the HDF5 dataset
            #--------------------------------------------------------
...
##############################################################################################################################################
##############################################################################################################################################
#############################################       PyVista Snapshot H5 Maker       ##########################################################
##############################################################################################################################################
##############################################################################################################################################
class HDF5PyVistaSnapshotWriter:
    """
    Create and read an independent HDF5 file for PyVista wavefield playback.

    This writer intentionally stores the heavy 2D wavefield snapshots outside the
    training HDF5 dataset. Each sample group contains the same keys expected by
    PyVistaWavePropagationQtViewer, so a stored sample can be loaded and passed
    directly into:

        viewer = PyVistaWavePropagationQtViewer(snapshots)
    """

    def __init__(self, h5_path, compression="gzip", compression_opts=4, snapshot_dtype=np.float32):
        self.h5_path = Path(h5_path)
        self.compression = compression
        self.compression_opts = compression_opts
    ...
    def _sample_key(self, sample):
        """
        Return the stable HDF5 group name for one simulation sample.
        """
        return f"sample_{int(sample):06d}"

    def _write_array_dataset(self, group, name, array_value, dtype=None, compress=True):
        """
        Write one array dataset inside a fresh sample group.
        """
        array_value = np.asarray(array_value, dtype=dtype)
    ...
    def append_sample(self, snapshots, sample, sample_metadata=None, overwrite=True):
        """
        Store one PyVista-ready snapshot dictionary in the snapshot HDF5 file.

        Parameters
        ----------
        snapshots : dict
            Dictionary returned by animation2D_FDM.animate() when store_snapshots=True.

        sample : int
            Simulation sample identifier used to create a stable HDF5 group name.

        sample_metadata : dict, optional
            Extra velocity-model metadata to store as attributes.

        overwrite : bool, optional
            If True, replacing the same sample is allowed. This is convenient while
            tuning visualization parameters without accumulating duplicate samples.
        """
        required_keys = [
            "wavefield_snapshots",
        ...
        missing_keys = [key for key in required_keys if key not in snapshots]
        if missing_keys:
            raise KeyError(f"Missing required PyVista snapshot keys: {missing_keys}")

        self.h5_path.parent.mkdir(parents=True, exist_ok=True)
        sample_key = self._sample_key(sample)
        sample_metadata = {} if sample_metadata is None else dict(sample_metadata)

        with h5py.File(self.h5_path, "a") as h5f:
            h5f.attrs["file_type"] = "pyvista_wavefield_snapshots"
            h5f.attrs["schema_version"] = "1.0"

            samples_group = h5f.require_group("samples")

            if sample_key in samples_group:
                if not overwrite:
                    raise ValueError(f"Sample '{sample_key}' already exists in {self.h5_path}.")
                del samples_group[sample_key]

            sample_group = samples_group.create_group(sample_key)
            sample_group.attrs["sample"] = int(sample)
            sample_group.attrs["dx"] = float(snapshots["dx"])
            sample_group.attrs["dz"] = float(snapshots["dz"])
            sample_group.attrs["dt"] = float(snapshots["dt"])
            sample_group.attrs["nx"] = int(snapshots["nx"])
            sample_group.attrs["nz"] = int(snapshots["nz"])
            sample_group.attrs["nt"] = int(snapshots["nt"])
            sample_group.attrs["source_x"] = int(snapshots["source_x"])
            sample_group.attrs["source_z"] = int(snapshots["source_z"])
            #--------------------------------------------------------
            if "n_source_events" in snapshots:                                                   # Store the rupture source-event count when snapshot metadata provides it
                sample_group.attrs["n_source_events"] = int(snapshots["n_source_events"])        # Keep the source-event count as a lightweight group attribute
            #--------------------------------------------------------
            sample_group.attrs["model_type"] = str(snapshots["model_type"])

            self._write_array_dataset(
                sample_group,
                "wavefield_snapshots",
                snapshots["wavefield_snapshots"],
            ...
            self._write_array_dataset(
                sample_group,
                "snapshot_time_indices",
                snapshots["snapshot_time_indices"],
                dtype=np.int32,
                compress=True,
            )
            self._write_array_dataset(
            ...
            self._write_array_dataset(
                sample_group,
                "receiver_z",
                snapshots["receiver_z"],
                dtype=np.int32,
                compress=False,
            )
            #--------------------------------------------------------
            if "source_x_all" in snapshots:                                                       # Store all rupture source x indices when available
                self._write_array_dataset(sample_group, "source_x_all", snapshots["source_x_all"], dtype=np.int32, compress=False)  # Write source x geometry for PyVista playback
            if "source_z_all" in snapshots:                                                       # Store all rupture source z indices when available
                self._write_array_dataset(sample_group, "source_z_all", snapshots["source_z_all"], dtype=np.int32, compress=False)  # Write source z geometry for PyVista playback
            if "source_delay_s" in snapshots:                                                     # Store all rupture source delays in seconds when available
                self._write_array_dataset(sample_group, "source_delay_s", snapshots["source_delay_s"], dtype=np.float64, compress=False)  # Write source delays for PyVista playback
            if "source_delay_samples" in snapshots:                                               # Store all rupture source delays in samples when available
                self._write_array_dataset(sample_group, "source_delay_samples", snapshots["source_delay_samples"], dtype=np.int32, compress=False)  # Write integer source delays for PyVista playback
            if "source_amplitude" in snapshots:                                                   # Store all rupture source amplitudes when available
                self._write_array_dataset(sample_group, "source_amplitude", snapshots["source_amplitude"], dtype=np.float64, compress=False)  # Write source amplitude multipliers for PyVista playback
            if "source_polarity" in snapshots:                                                    # Store all rupture source polarities when available
                self._write_array_dataset(sample_group, "source_polarity", snapshots["source_polarity"], dtype=np.float64, compress=False)  # Write source polarity multipliers for PyVista playback
            #--------------------------------------------------------

            metadata_group = sample_group.create_group("sample_metadata")
    ...
    def list_samples(self):
        """
        Return the stored sample group names.
        """
        if not self.h5_path.exists():
            return []
        ...
        with h5py.File(self.h5_path, "r") as h5f:
            if "samples" not in h5f:
                return []
            return sorted(h5f["samples"].keys())

    def read_sample(self, sample):
        """
        Load one sample as a dictionary ready for PyVistaWavePropagationQtViewer.
        """
        sample_key = self._sample_key(sample) if isinstance(sample, (int, np.integer)) else str(sample)

        with h5py.File(self.h5_path, "r") as h5f:
            sample_group = h5f["samples"][sample_key]
            attrs = sample_group.attrs

            snapshots = {
                "wavefield_snapshots": sample_group["wavefield_snapshots"][...],
                "snapshot_time_indices": sample_group["snapshot_time_indices"][...],
                "snapshot_times": sample_group["snapshot_times"][...],
                "velocity_model": sample_group["velocity_model"][...],
                "dx": float(attrs["dx"]),
                "dz": float(attrs["dz"]),
                "dt": float(attrs["dt"]),
                "nx": int(attrs["nx"]),
                "nz": int(attrs["nz"]),
                "nt": int(attrs["nt"]),
                "source_x": int(attrs["source_x"]),
                "source_z": int(attrs["source_z"]),
                "receiver_x": sample_group["receiver_x"][...],
                "receiver_z": sample_group["receiver_z"][...],
                "model_type": str(attrs["model_type"]),
            }
            #--------------------------------------------------------
            if "n_source_events" in attrs:                                                       # Read the rupture source-event count when present
                snapshots["n_source_events"] = int(attrs["n_source_events"])                     # Restore the source-event count from group attributes
            if "source_x_all" in sample_group:                                                   # Read all rupture source x indices when present
                snapshots["source_x_all"] = sample_group["source_x_all"][...]                    # Restore all source x indices for PyVista playback
            if "source_z_all" in sample_group:                                                   # Read all rupture source z indices when present
                snapshots["source_z_all"] = sample_group["source_z_all"][...]                    # Restore all source z indices for PyVista playback
            if "source_delay_s" in sample_group:                                                 # Read all rupture source delays in seconds when present
                snapshots["source_delay_s"] = sample_group["source_delay_s"][...]                # Restore all source delays in seconds for PyVista playback
            if "source_delay_samples" in sample_group:                                           # Read all rupture source delays in samples when present
                snapshots["source_delay_samples"] = sample_group["source_delay_samples"][...]    # Restore all source delays in samples for PyVista playback
            if "source_amplitude" in sample_group:                                               # Read all rupture source amplitudes when present
                snapshots["source_amplitude"] = sample_group["source_amplitude"][...]            # Restore all source amplitude multipliers for PyVista playback
            if "source_polarity" in sample_group:                                                # Read all rupture source polarities when present
                snapshots["source_polarity"] = sample_group["source_polarity"][...]              # Restore all source polarity multipliers for PyVista playback
            #--------------------------------------------------------

            if "sample_metadata" in sample_group:
                snapshots["sample_metadata"] = dict(sample_group["sample_metadata"].attrs)
    ...
    def compare_receiver(
        ...
        if show_velocity_models:
            ...
            for ax, item in zip(axes, loaded):
                ...
                ax.set_xlabel("ix", fontsize=textsize)
                ax.set_ylabel("iz", fontsize=textsize)
                cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
                cbar.set_label("Velocity [m/s]",fontsize=textsize,)
                cbar.ax.tick_params(labelsize=textsize)
                
                ax.tick_params(axis="both", labelsize=textsize)
            
            safe_img_dic = Path(safe_img_dic)
            fig_name_pdf = f"velocity_models_comparison_receiver_ST{receiver_id + 1}_model_{subname}.pdf"
            fig_path_pdf = safe_img_dic / fig_name_pdf
            fig_name_svg = f"velocity_models_comparison_receiver_ST{receiver_id + 1}_model_{subname}.svg"
            fig_path_svg = safe_img_dic / fig_name_svg
            
            plt.savefig(fig_path_pdf, dpi=200, bbox_inches="tight", pad_inches=0.05)
            plt.savefig(fig_path_svg, dpi=200, bbox_inches="tight", pad_inches=0.05)
            plt.show()
...
class ReceiverConditionedLossHistoryReviewer:                                                    # Class to load, summarize, and plot receiver-conditioned CNN loss histories
    ...
    def export_dashboard_data_h5(self, h5_path=None):                                             # Export the dashboard source data to an HDF5 file
        if self.history is None:                                                                  # Ensure the NPZ history has already been loaded
            self.load_history()                                                                   # Load history if this method is called directly

        if h5_path is None:                                                                       # Use default output path if user does not provide one
            h5_path = self.outputs_dir / "receiver_conditioned_cnn_loss_dashboard_data_FourierOnet.h5"
        else:
            h5_path = Path(h5_path)

        h5_path.parent.mkdir(parents=True, exist_ok=True)                                         # Create output folder if needed

        def _write_dataset(group, name, value):                                                   # Small local helper to overwrite one dataset
            if value is None:
                return
            if name in group:
                del group[name]
        ...
        with h5py.File(h5_path, "w") as h5f:                                                      # Create a fresh HDF5 export file
            h5f.attrs["source_npz_path"] = str(self.loss_history_path)                            # Store source NPZ path for traceability
            h5f.attrs["export_created_unix_time"] = float(time.time())                            # Store export timestamp
            h5f.attrs["num_epochs"] = int(self.num_epochs)                                       # Store number of epochs
            h5f.attrs["best_epoch"] = int(self.best_epoch)                                       # Store best epoch
            h5f.attrs["best_val_loss"] = float(self.best_val_loss)                               # Store best validation loss
            if self.batch_size_query is not None:
                h5f.attrs["batch_size_query"] = int(self.batch_size_query)                       # Store query batch size when available

            g1 = h5f.create_group("panel_1_epoch_loss")                                          # Data for subplot 1
            _write_dataset(g1, "epochs", self.epochs)
            _write_dataset(g1, "train_losses", self.train_losses)
            _write_dataset(g1, "val_losses", self.val_losses)
            g1.attrs["best_epoch"] = int(self.best_epoch)
            g1.attrs["best_val_loss"] = float(self.best_val_loss)

            g2 = h5f.create_group("panel_2_generalization_gap")                                  # Data for subplot 2
            _write_dataset(g2, "epochs", self.epochs)
            _write_dataset(g2, "gap_losses", self.gap_losses)

            g3 = h5f.create_group("panel_3_epoch_loss_semilog")                                  # Data for subplot 3
            _write_dataset(g3, "epochs", self.epochs)
            _write_dataset(g3, "train_losses", self.train_losses)
            _write_dataset(g3, "val_losses", self.val_losses)
            g3.attrs["best_epoch"] = int(self.best_epoch)
            g3.attrs["best_val_loss"] = float(self.best_val_loss)

            g4 = h5f.create_group("panel_4_train_batch_loss_per_epoch")                          # Data for subplot 4
            _write_dataset(g4, "train_batch_losses", self.train_batch_losses)
            _write_dataset(g4, "train_batch_steps", self.train_batch_steps)
            _write_dataset(g4, "train_batch_epochs", self.train_batch_epochs)
            _write_dataset(g4, "train_batch_step_in_epoch", self.train_batch_step_in_epoch)
            g4.attrs["available"] = bool(
                self.train_batch_losses is not None
                and self.train_batch_epochs is not None
                and self.train_batch_step_in_epoch is not None
            )

            g5 = h5f.create_group("panel_5_epoch_time")                                          # Data for subplot 5
            _write_dataset(g5, "epochs", self.epochs)
            _write_dataset(g5, "epoch_times_sec", self.epoch_times_sec)
            g5.attrs["available"] = bool(self.epoch_times_sec is not None)

            g6 = h5f.create_group("panel_6_summary")                                             # Data for subplot 6
            g6.attrs["num_epochs"] = int(self.num_epochs)
            g6.attrs["best_epoch"] = int(self.best_epoch)
            g6.attrs["best_val_loss"] = float(self.best_val_loss)
            g6.attrs["initial_train_loss"] = float(self.train_losses[0])
            g6.attrs["final_train_loss"] = float(self.train_losses[-1])
            g6.attrs["initial_val_loss"] = float(self.val_losses[0])
            g6.attrs["final_val_loss"] = float(self.val_losses[-1])
            g6.attrs["final_gap"] = float(self.gap_losses[-1])
            if self.batch_size_query is not None:
                g6.attrs["batch_size_query"] = int(self.batch_size_query)

        print("\n" + "=" * 120)                                                                   # Print separator line
        print("Receiver-conditioned loss dashboard data exported successfully")                   # Print confirmation message
        print(h5_path)                                                                           # Print exported HDF5 path
        print("=" * 120)                                                                          # Print separator line
        print("\n")                                                                               # Print blank line

        return h5_path                                                                            # Return exported HDF5 path for downstream scripts


    def run_all(self, export_h5=True, h5_path=None):                                             # Execute complete review workflow
        self.load_history()                                                                       # Load NPZ data
        self.print_summary()                                                                      # Print numerical summary
        if export_h5:                                                                             # Optionally export source data before plotting
            self.export_dashboard_data_h5(h5_path=h5_path)                                       # Save dashboard source arrays to HDF5
